...
public class ConcurrentSkipListMap<K,V> extends AbstractMap<K,V>
    ...
    static class Index<K,V> {
         ...
         * @return true if successful
         *
         * 将当前节点的右节点指向succ的右节点,也就是删除succ节点,并将当前节点的右节点指向succ的右节点
         *   A->B-C
         *to: A->C
         *
         */
     ...
     *
     * 翻译上面的注释
     * 返回一个最底层的node节点，其key肯定小于需要查找的key，
     * 或者如果没有这样的节点，则返回基本级别的头。
     * 还会删除沿途找到的已删除节点的索引。调用者依赖于清除已删除节点的索引的副作用。
     *
     */
    private Node<K,V> findPredecessor(Object key, Comparator<? super K> cmp) {
        ...
        for (;;) {
            for (Index<K,V> q = head, r = q.right, d;;) { // 每次重头开始找
                // q = 7 r = 20
                if (r != null) {
                    Node<K,V> n = r.node;
                    K k = n.key;
                    if (n.value == null) {
                        if (!q.unlink(r))  // 删除r节点，如果失败，重新找（跳到上一层循环出现开始），这是处理并发问题
                            break;           // restart
                        r = q.right;         // reread r
                        continue;
                    }
                  /**
                   * 如果key大于当前节点的key，继续往右找，
                   */
                  if (cpr(cmp, key, k) > 0) {
                        q = r;
                        r = r.right;
                ...
                if ((d = q.down) == null) // 如果没有下层点，返回当前节点 这时候不一定就是要找到的节点
                    return q.node;
                q = d; // 如果有下层节点，继续往下找
                r = d.right;
            }
    ...
    private V doGet(Object key) {
        ...
        outer: for (;;) {
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                ...
                Object v; int c;
                if (n == null) // 没有下一个节点, 说明key比全部的节点都大，key不存在
                    break outer;
                Node<K,V> f = n.next;
                if (n != b.next)                // inconsistent read 并发问题,说明b节点的next被修改过 重新查找
                    break;
                if ((v = n.value) == null) {    // n is deleted 并发问题
                    n.helpDelete(b, f);
                    break;
                }
                if (b.value == null || v == n)  // b is deleted 并发问题
                    break;
                if ((c = cpr(cmp, key, n.key)) == 0) { // 下一节点和key相等，找到了
                    @SuppressWarnings("unchecked") V vv = (V)v;
                    return vv;
                }
                if (c < 0) // 一下节点的key比key大，说明key大于上一节点，小于下一节点，而链表是有序的，所以key不存在
                    break outer;
                b = n; // 如果下一节点的key比key小，继续往下找
                n = f;
            }
        }
        return null;
    ...
    private V doPut(K key, V value, boolean onlyIfAbsent) {
        ...
        if (key == null)
            throw new NullPointerException();
        Comparator<? super K> cmp = comparator;
        // 找到底层的前驱节点，如果发现key已经存在，就替换value
        outer: for (;;) {
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                if (n != null) {
                    Object v; int c;
                    Node<K,V> f = n.next;
                    if (n != b.next)               // inconsistent read  并发问题
                        break;
                    if ((v = n.value) == null) {   // n is deleted  并发问题?
                        n.helpDelete(b, f);
                        break;
                    ...
                    if (c == 0) {
                        ...
                        break; // restart if lost race to replace value // 并发自旋
                    }
                ...
                if (!b.casNext(n, z))
                    break;         // restart if lost race to append to b // 并发自旋
                break outer;
            }
        }

//      原本代码  int rnd = ThreadLocalRandom.nextSecondarySeed();
      ...
      // 防止报错代码
        ...
        // 随机确定新节点的层数, 大概25%的概率
        //0x80000001是16进制 等于 10000000000000000000000000000001
        // 只要生成的随机数的最高位和最低位都是0，就等于0 50% * 50%
        if ((rnd & 0x80000001) == 0) { // test highest and lowest bits
            int level = 1, max;
            while (((rnd >>>= 1) & 1) != 0) // 50%的概率level+1
                ++level;
            Index<K,V> idx = null;
            HeadIndex<K,V> h = head;
            if (level <= (max = h.level)) {
                for (int i = 1; i <= level; ++i) // 将最底层到最上层的index都建立好，并且通过down关联
                    idx = new Index<K,V>(z, idx, null);
            }
            else { // try to grow by one level 如果level大于当前的最大层数，就增加一层
                ...
                for (;;) {
                    ...
                    for (int j = oldLevel+1; j <= level; ++j) // 增加的层数直接关联新插数据
            ...
            splice: for (int insertionLevel = level;;) {
              int j /** 当前level  */ = h.level;
                for (Index<K,V> q = h, r = q.right, t = idx;;) {
                    ...
                    if (j == insertionLevel) { // 如果当前的level等于 要插入节点的level
                        if (!q.link(r, t)) // 插入节点  A->B   A->C->B
                            break; // restart  插入失败头开始重新插入
                        if (t.node.value == null) { // ?什么情况下会出现这种情况
                            findNode(key); // 遍历清楚已删除的节点
                            break splice;
                        }
                        if (--insertionLevel == 0) // 如果到了第一层，说明已经将index节点插入到全部层级
                            break splice;
                    }

                    if (--j >= insertionLevel && j < level) // 如果当前的level大于等于要插入的level，且小于等于最大的level(说明t的index比当前index大)
                        t = t.down;
                    q = q.down;
                    r = q.right;
                }
